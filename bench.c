...
typedef struct thread_data {
	long uniq_id;
	hash_list_t **p_p_hash_list;
	struct barrier *barrier;
	unsigned long nb_add;
	unsigned long nb_remove;
	unsigned long nb_contains;
	unsigned long nb_found;
	unsigned long nb_expand;
	unsigned long nb_shrink;
...
static hash_list_t *g_p_hash_list;
...
static void global_init(int n_threads, int rlu_max_ws) {
	RLU_INIT(RLU_TYPE_COARSE_GRAINED, rlu_max_ws);
	RCU_INIT(n_threads);
}
...
static hash_list_t *hash_list_init(int n_buckets) {
#ifdef IS_RLU
	return rlu_new_hash_list(n_buckets);
#else
#ifdef IS_RCU
	return rcu_new_hash_list(n_buckets);
#else
...
#ifdef IS_RLU
	return rlu_hash_list_contains(d->p_rlu_td, *d->p_p_hash_list, key);
#else
#ifdef IS_RCU
	return rcu_hash_list_contains(d->p_p_hash_list, key);
#else
	printf("ERROR: benchmark not defined!\n");
	abort();
#endif
...
#ifdef IS_RLU
	return rlu_hash_list_add(d->p_rlu_td, *d->p_p_hash_list, key);
#else
#ifdef IS_RCU
	return rcu_hash_list_add(d->p_p_hash_list, key);
#else
	printf("ERROR: benchmark not defined!\n");
	abort();
#endif
...
#ifdef IS_RLU
	return rlu_hash_list_remove(d->p_rlu_td, *d->p_p_hash_list, key);
#else
#ifdef IS_RCU
	return rcu_hash_list_remove(d->p_p_hash_list, key);
#else
	printf("ERROR: benchmark not defined!\n");
	abort();
#endif
...
static int hash_list_expand(thread_data_t *d) {
	d->nb_expand++;
#ifdef IS_RLU
	return rlu_hash_list_expand(d->p_rlu_td, *d->p_p_hash_list);
#else
#ifdef IS_RCU
	return rcu_hash_list_expand(d->p_p_hash_list);
#else
	printf("ERROR: benchmark not defined!\n");
	abort();
#endif
...
static int hash_list_shrink(thread_data_t *d) {
	d->nb_shrink++;
#ifdef IS_RLU
	return rlu_hash_list_shrink(d->p_rlu_td, *d->p_p_hash_list);
#else
#ifdef IS_RCU
	return rcu_hash_list_shrink(d->p_p_hash_list);
#else
	printf("ERROR: benchmark not defined!\n");
	abort();
#endif
...
static void *test(void *data)
{
	...
	if (d->uniq_id == 0) {
		...
		int size = hash_list_size(*d->p_p_hash_list);
		printf("Hash-list size     : %d\n", size);	
		
/*#ifdef IS_RCU
		hash_list_print_stats(d->p_hash_list);
		rcu_hash_list_expand(&d->p_hash_list);
		hash_list_print_stats(d->p_hash_list);
		rcu_hash_list_shrink(&d->p_hash_list);
		hash_list_print_stats(d->p_hash_list);
#else
#ifdef IS_RLU
		hash_list_print_stats(d->p_hash_list);
		rlu_hash_list_expand(d->p_rlu_td, d->p_hash_list);
		hash_list_print_stats(d->p_hash_list);
		rlu_hash_list_shrink(d->p_rlu_td, d->p_hash_list);
		hash_list_print_stats(d->p_hash_list);
...
int main(int argc, char **argv)
{
	...
	// These options don't set a flag
			...
			{"resize-rate",               required_argument, NULL, 'g'},
			{NULL, 0, NULL, 0}
	};

	int i, c, val, size, ret;
	unsigned long reads, updates, expands, shrinks;
	thread_data_t *data;
	pthread_t *threads;
	pthread_attr_t attr;
	barrier_t barrier;
	...
	int resize = DEFAULT_RESIZE;
	int alternate = 1;
	sigset_t block_set;

	while(1) {
		i = 0;
		c = getopt_long(argc, argv, "hab:d:i:n:r:s:u:w:g:", long_options, &i);
		...
		switch(c) {
			...
			printf("intset "
				...
				"  -g, --resize-rate <int>\n"
				"        Percentage of resizes (1000 = 100 percent) (default=" XSTR(DEFAULT_RESIZE) ")\n"				
...
#ifdef IS_RCU
	if (update != 0) {
		printf("RCU has not support for concurrent updates (updates must be 0).\n");
		abort();
	}
#endif
	...
	g_p_hash_list = hash_list_init(n_buckets);
	...
	for (i = 0; i < nb_threads; i++) {
		...
		data[i].p_p_hash_list = &g_p_hash_list;
	...
	for (i = 0; i < nb_threads; i++) {
		...
		size += data[i].diff;
		expands += data[i].nb_expand;
		shrinks += data[i].nb_shrink;
	}
	printf("Set size      : %d (expected: %d)\n", hash_list_size(g_p_hash_list), size);
	...
	printf("#expands     : %lu (%f / s)\n", expands, expands * 1000.0 / duration);
	printf("#shrinks     : %lu (%f / s)\n", shrinks, shrinks * 1000.0 / duration);
	/* Cleanup */
	free(threads);
	free(data);
